...
### How is working the child/parent relationship of `Entidade`?

The crucial point of this relationship is the generic relationship between Entity and Pessoa.
And this is what makes TS to "understand" the entity type and maintain the type safety between parent and child.

1. Entity's Generic Signature

export default abstract class Entidade<Tipo, Props extends EntidadeProps>{

Here we have two generic parameters:

`Tipo`: The concrete type of the child entity, such as `Pessoa`
Props: The properties this entity receives

Which means that the `Entity` class does not know beforehand which concrete type will extend it, who defines it is the
child.

2. The child class defines the generics

export default class Pessoa extends Entidade<Pessoa, PessoaProps>{...}

Here is where the key moment happens

• The first generic argument (`Pessoa`) indicates that the concrete type is `Pessoa`
• The second argument (`PessoaProps`) indicates that the properties type is `PessoaProps`, which in this case have a name,
cpf and extends the `EntidadeProps` which says it also has an id

This creates a circular link (a auto referring type), typical in DDD

"An entity must know its own concrete type"

3. Why is it important? 

Thanks to this definition, TypeScript:

1. Knows that `clone()` will return an instance of `Pessoa`, and not only `Entidade`
2. Ensures that `clone()` only will accept properties of its concrete type
3. Gives automatic and safe typing, without having to override anythings in each child class.

Example:

```ts
const pessoa = new Pessoa({ nome: "Caio", cpf: "123" });
const outra = pessoa.clone({ nome: "Caio Ceretta" });

//TS knows `outra` is a `Pessoa`
// "novasProps" accepts only `PessoaProps`
// if we pass an inexistent property, it will 
```

This works because of the generic `Tipo`. Without it, `clone` would have to return `Entidade<any, any>` completely
losing the information of the specific entity type

4. Execution Relation

1. `Pessoa` inherits the structure and behavior of the class `Entidade`
2. Generics make ts "replace" Entidade<Pessoa, PessoaProps>
3. This way, inside `Entidade`, any reference to `Tipo`, or `Props` will be solved in a strongly typed manner.

5. Quick Resume

Generic `Tipo`: Tells `Entidade` which is the concrete type being created
Generic `Props`: Tells `Entidade` which are the required/expected fields (PessoaProps)
Benefit: Allows generic methods, such as `clone` or `igual`, to know which is the correct return type and avoid typing
errors.
Practical Result: Reusable, safe, and with strong typing without the need to rewrite methods on each entity
...
### Why are the nome and cpf optional in class `Pessoa`? 
...
• Id is treated in a special way, if i does not exist, a new Id is automatically generated.
• props is stored in the instance, but it does not do any validation about what fields are required, that is up to the
concrete class


● The reason they are optional is to satisfy typescript and allow methods like clone without breaking its typings.
  • Clone needs to create an instance of any entity, merging the original props with the new props.
  • For TS, if `PessoaProps` had a name and a cpf as required, `clone` would generate a typing error because `novasProps`
  may not include them.
  • Setting the `nome` and `cpf` as optional, when merging (...this.props, ...novasProps) ensures that the Pessoa constructor
  still receives the correct value, since this,props always will have the original value.

● Conclusion
. Id is autogenerated if it does not exist. This is safe for every entity
. cpf and nome are not generated automatically, there is no valid cpf or valid name that can be generated automatically and
can added to the domain.
. This is the reason the Pessoa constructor delegates their validation to the object values that will throw errors if the
values are invalid or undefined.
